...
import { Search, Filter, MapPin, Clock, Briefcase, ChevronRight, X, Star } from 'lucide-react';
...
const LICENSES = ['EASA Part 66 B1.1','EASA Part 66 B1.2','EASA Part 66 B2','EASA Part 66 C','FAA A&P','GCAA CAR 66','CAAS','CAAM','DGCA','No license required','Other'];
  ...
  const fetchJobs = async () => {
    ...
    try {
      const { data } = await supabase
        .from('job_posts')
        .select('*, company_profiles(entity_name, logo_url, location, username)')
        .eq('is_active', true)
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                {activeFiltersCount > 0 && (
                  <button onClick={() => setFilters({ categories: [], departments: [], job_types: [], licenses: [] })}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {job.categories?.slice(0,2).map((c: string) => (
                          <span key={c} className="px-1.5 py-0.5 bg-blue-500/20 text-blue-300 rounded text-xs">{c}</span>
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            {selectedJob && (
              <div className="lg:col-span-3 bg-gray-800 rounded-xl border border-gray-700 p-6 h-fit sticky top-20">
                ...
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-14 h-14 rounded-lg bg-blue-600 flex items-center justify-center text-white text-xl font-bold flex-shrink-0 overflow-hidden">
                    {selectedJob.company_profiles?.logo_url
                      ...
                      : selectedJob.company_profiles?.entity_name?.charAt(0) || '?'}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                  {selectedJob.licenses?.length > 0 && (
                    <div><p className="text-xs text-gray-500 mb-1">License required</p><p className="text-gray-300 text-xs">{selectedJob.licenses.join(', ')}</p></div>
                  )}
                  {selectedJob.aircraft_types && (
                    <div><p className="text-xs text-gray-500 mb-1">Aircraft / Component</p><p className="text-gray-300 text-xs">{selectedJob.aircraft_types}</p></div>
                  )}
                  {(selectedJob.experience_min !== null || selectedJob.experience_max !== null) && (
                    <div><p className="text-xs text-gray-500 mb-1">Experience</p><p className="text-gray-300 text-xs">{selectedJob.experience_min || 0}–{selectedJob.experience_max || '+'} years</p></div>
                  )}
                  {selectedJob.working_hours && (
                    <div><p className="text-xs text-gray-500 mb-1">Working hours</p><p className="text-gray-300 text-xs">{selectedJob.working_hours}</p></div>
                  )}
                  {selectedJob.show_salary && selectedJob.salary_range && (
                    <div><p className="text-xs text-gray-500 mb-1">Salary</p><p className="text-gray-300 text-xs">{selectedJob.salary_range}</p></div>
                  )}
                  {(selectedJob.visa_sponsorship || selectedJob.visa_requirements) && (
                    <div><p className="text-xs text-gray-500 mb-1">Visa / Work permit</p>
                      <p className="text-gray-300 text-xs">{selectedJob.visa_sponsorship ? '✓ Sponsorship available' : 'No sponsorship'}{selectedJob.visa_requirements ? ` · ${selectedJob.visa_requirements}` : ''}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2 pt-4 border-t border-gray-700">
                  ...
                  {selectedJob.apply_inhouse && (
                    <button className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium transition-colors">
                      Apply via AECircle
                    </button>
                  )}
                  {selectedJob.apply_external_link && (
                    <a href={selectedJob.apply_external_link} target="_blank" rel="noreferrer"
                      ...
                      View job on company site <ChevronRight className="w-4 h-4" />
                    </a>
                  )}
                  {selectedJob.apply_email && (
                    <a href={`mailto:${selectedJob.apply_email}?subject=Application for ${selectedJob.title} (${selectedJob.job_code})`}
                      className="w-full flex items-center justify-center gap-2 py-2.5 border border-gray-600 text-gray-300 hover:text-white rounded-md text-sm transition-colors">
                      Send CV by email
                    </a>
                  )}
