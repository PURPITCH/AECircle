      ...
      <div className="max-w-5xl mx-auto px-4">
        <div className="flex items-center gap-3 h-14">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {navItem('/co/exams', 'Exams', GraduationCap)}
          </div>
...
export const CompanyExams: React.FC = () => {
  const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const { data: examData } = await supabase.from('exam_posts').select('*').eq('company_id', company.id).order('created_at', { ascending: false });
      setExams(examData || []);
      setLoading(false);
    };
    fetch();
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-semibold text-blue-400">Exam Listings</h2>
              <p className="text-xs text-gray-500 mt-0.5">{exams.length} listing{exams.length !== 1 ? 's' : ''}</p>
            </div>
          ...
          {exams.length === 0 ? (
            <div className="text-center py-16 text-gray-500">
              ...
              <p className="text-sm font-medium">No exam listings yet</p>
              <p className="text-xs mt-1">List your exam sessions to reach Part 66 candidates.</p>
              <button onClick={() => navigate('/co/exams/post')}
            ...
            <div className="space-y-3">
              {exams.map(exam => (
                <div key={exam.id} className="border border-gray-600 rounded-lg p-4 hover:border-gray-500 transition-colors">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-lg bg-blue-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0 overflow-hidden">
                      {companyLogo ? <img src={companyLogo} alt="" className="w-full h-full object-cover" /> : companyName.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p onClick={() => navigate(`/exams/${exam.exam_code}`)}
                            className="font-semibold text-white hover:text-blue-400 cursor-pointer transition-colors">
                            {exam.title}
                          </p>
                          <p className="text-xs text-gray-500 mt-0.5">{exam.location} · {exam.authority_approval}</p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <span className="text-xs font-mono text-gray-500 block">{exam.exam_code}</span>
                          <span className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${exam.is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                            {exam.is_active ? 'Active' : 'Inactive'}
                          </span>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {exam.categories?.slice(0,5).map((c: string) => <span key={c} className="px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded-full text-xs">{c}</span>)}
                      </div>
                    </div>
                  </div>
