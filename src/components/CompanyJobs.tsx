...
export const CompanyJobs: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const { data: company } = await supabase.from('company_profiles').select('entity_name, id').eq('user_id', user.id).maybeSingle();
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{jobs.length} post{jobs.length !== 1 ? 's' : ''}</p>
            </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {job.location}{job.is_remote ? ' · Remote available' : ''} · {job.job_types?.join(', ')}
                      </p>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {job.categories?.map((c: string) => (
                          <span key={c} className="px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded-full text-xs">{c}</span>
                        ))}
                        {job.licenses?.slice(0,2).map((l: string) => (
                          <span key={l} className="px-2 py-0.5 bg-gray-600 text-gray-300 rounded-full text-xs">{l}</span>
                        ))}
                      </div>
                    </div>
