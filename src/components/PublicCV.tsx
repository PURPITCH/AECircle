...
{/* Additional Information */}
        {(profile.availability || profile.notice_period || profile.driving_license_category) && (
          <Section title="Additional Information">
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
              {profile.availability && (
                <span><span className="text-gray-500">Status:</span> <span className="text-gray-800">{profile.availability}</span></span>
              )}
              {profile.notice_period && (
                <span><span className="text-gray-500">Notice:</span> <span className="text-gray-800">{profile.notice_period}</span></span>
              )}
              {profile.willing_to_relocate && (
                <span><span className="text-gray-500">Relocation:</span> <span className="text-gray-800">Yes{profile.preferred_locations ? ` — ${profile.preferred_locations}` : ''}</span></span>
              )}
            </div>
            {profile.driving_license_category && (
              <p className="text-sm mt-1">
                <span className="text-gray-500">Driving license:</span> <span className="text-gray-800">{profile.driving_license_category}{profile.driving_license_country ? ` — ${profile.driving_license_country}` : ''}</span>
              </p>
            )}
          </Section>
        )}
